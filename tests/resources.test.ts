...
test("doctor exposes enforced aggregate limits and live kernel counters without a browser", async () => {
  ...
  try {
    ...
    expect(result.resources.limits.cpuCores).toBeLessThanOrEqual(1);
    expect(result.resources.limits.memoryBytes).toBeLessThanOrEqual(2147483648);
    expect(result.resources.limits.swapBytes).toBe(0);
    expect(result.resources.limits.tasks).toBeLessThanOrEqual(512);
