...
- Redesigned the panel as a single pill with the session count, tinted by state, pulsing once when a session or an application appears, expanding on hover into one card per session. A right click toggles desktop notifications and the working frame, opens the viewer or quits; settings persist in `~/.config/sbar-orbit/panel.json`. The pill lands on the largest monitor unless one is named by index or connector. The frame is on by default.
