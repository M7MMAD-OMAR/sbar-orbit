...
- Added the working indicator, `sbar-orbit panel --indicator`: a click-through overlay frame around the monitor while any session's action is in flight, drawn once per state change and transparent above the person's own `gtk.css`. Verified inside a private display with edge pixels sampled during and after a slow launch. The panel now takes the person's GTK colour names when their theme defines them.
- Profiles are removed when a session stops and a broker's workspace when it closes; `sbar-orbit clean` removes workspaces whose broker is gone, keeping a live broker's directory by its recorded pid and command line. Before this every test and experiment profile was retained forever, 4.5 GB on this workstation.
