...
```sh
sbar-orbit panel                  # the project mark on the largest screen, right edge
...
Resting the pointer on it opens one card per session: agent, a state chip, task, what is on screen, which tab or window of how many, and the current action, with one row of two actions under the whole list: open the viewer, and the settings. There used to be a third, "a browser of my own", which created a paused session and then opened the viewer on it. It was removed: next to a button that opens the viewer it read as a choice between the viewer and the person's OWN browser, which is the one thing Orbit never touches, and both buttons ended in the same place anyway. Driving a private browser by hand is still available from inside the viewer, where every other session action already lives. Clicking a card opens the viewer, a left click on the capsule opens the viewer, a right click opens the settings. Opening waits 220 ms and closing waits 320 ms, so a pointer crossing the edge of the screen on its way elsewhere does not flash the whole list. The surface is pinned to one end of its edge with a margin that puts the capsule where the person left it, because a surface anchored to a single edge is centred by the compositor and the capsule would then jump by half the height of the cards every time they opened. The card keeps its place in the layout even while closed, so the surface never resizes under the pointer; what changes is the surface's input region, which is tightened back onto the capsule so a closed panel swallows no clicks in the card-sized rectangle beside it.

<img src="images/panel-card-on-desktop.jpg" alt="One session card open against the right edge of the desktop: the agent name Claude, a running chip, the task, the application with its window count and the pointer, and the three actions under it" width="720">

One card, open on the development host on 19 September 2026. It reads, in order: who is working, the
state chip, the task the person gave, then the session itself, an application with one window of one
open and a pointer that exists only inside it. The mark the pointer is resting on is the sliver against
the right edge of the screen, and the rest of the frame is the person's own desktop, untouched while the
agent works. What the frame does not show is a second session, so it does not show how the list stacks,
and it costs nothing to produce: everything on the card comes from `session.presence`, never from a
capture.
...
Four layer surfaces on the overlay layer, one strip along each edge of the same monitor, each with an empty input region so every click, hover and edge gesture passes through to whatever is underneath. Together they wash the screen edges while at least one session's current action is in flight and are hidden otherwise, so a glance at any edge says whether an agent is working right now. It is a glow rather than a border: each strip is 120 px deep and carries a gradient whose stops are bunched against the edge and thin out over the rest of that depth, so the glow covers a large part of the screen while the part with any weight in it stays out at the edge, gone before it reaches anything being read, in Orbit's `#255fce`, the same blue the agent pointer is drawn in, because a wash laid over a wallpaper nobody chose cannot borrow its contrast from a palette. `frameColor` set to `accent` asks for the desktop's own `accent_bg_color` instead, and follows the theme in light and dark. The four strips overlap at the corners and the overlap is wanted: two soft washes crossing is what gives a corner its vignette instead of a mitred join. Four strips cost about 3.8 MiB of buffer between them on a 2560 by 1440 output; one surface covering the output with a transparent centre would cost 29 MB per buffer on a 2560 by 1600 display at its scale and 75 MB on the 4K one, uploaded again at every map. The strips are painted by a rule loaded above user priority, since the person's `gtk.css` may paint every window in their theme's colour.

It breathes while working, and that was measured before it shipped, because this machine keeps compositor effects off: two displays at 4K and 2560 by 1600 with fractional scaling make them expensive, and this whole line of work started with a processor complaint. Measured by running the panel check twice each way, static against pulsing, and reading the panel process's own CPU ticks: **static 3 and 1 ticks, pulsing 16 and 16**. The pulse therefore costs about 15 ticks, 0.15 s of processor, across a capture in which the glow was shown for roughly five seconds, so on the order of 3% of one core while an agent is working and nothing at all while idle, since the strips are unmapped between state changes. The swing is small and the period long, 2.6 s, because a working indicator is looked at out of the corner of an eye for minutes at a time and should read as alive rather than as an alarm. `--no-pulse`, or the switch in the settings, keeps the glow and drops the motion.

The falloff is what the check tests, not a single pixel, because one pixel cannot tell a soft glow from the hard border this replaced. Sampling 1, 8 and 20 px in from the left and top edges and measuring each sample's distance from the capture's own centre: **101, 45, 9** on the left and **107, 50, 6** on the top while working, and flat at **26, 26, 26** once the action finished. A glow falls away inward; an edge with no glow is flat whatever the desktop behind it shows.
...
The widget is the project mark in the shell's own colours, drawn as a `Shape` from the same path the panel uses and tinted with `Appearance.colors`, so it follows the person's generated theme with nothing to configure. It shows the state by colour with no number on it, blinks once when a session or a window appears, lists the sessions on hover in the shell's own popup, and opens the viewer on click. Showing and hiding it is the bar layout's job, in the shell's own `Revealer`, the way every other indicator there is revealed, so a bar with no agents running looks exactly as it did before. It belongs beside the tray rather than among the status indicators: Orbit reports an application that is running, which is what a tray is for, and in the indicator group it crowded the notification badge and put a viewer click inside the button that opens the sidebar.
...
It reads `desktop/orbit-stream.py`, a helper that prints one JSON line whenever what a bar would show changes, through `desktop/orbit_client.py`, which is the broker client with no toolkit attached so a bar module never pulls in GTK. The helper is restarted if it exits, so the bar recovers on its own. Hovering the mark shows a popup listing the sessions; it is a tooltip and closes when the pointer leaves it, so the click target is the mark itself. A click runs `orbit-stream --open-viewer`, which since 14 September 2026 asks the broker to open the viewer in a window of Orbit's own with the panel's browser settings, rather than handing the link to `xdg-open`: that was the person's browser and profile, and from a shell whose PATH carried no `xdg-open` it opened nothing and said nothing. Measured from this desktop after the change: the window opened in Chromium as a window of its own.
...
Verified inside a private display, the same way the panel is: a second quickshell instance rendering the widget and the content of its popup, reading the live broker. The capsule drew in the theme's idle colour and the popup read `2 sessions · 5 windows` over one row per session with the agent, the task, the window count and the state. Re-verified after the capsule became the mark: a quickshell instance inside a private display drew the widget's own `Item` and `Shape`, verbatim from this file, in a stand-in bar row beside sixteen pixel glyphs. The mark centres on the row and is not clipped. Its size was chosen there rather than guessed: at 15 its filled square read lighter than the glyphs beside it, at 22 it overflowed the row, and 18 is the one that matches. The path is carried with the mark's safe area removed, so every coordinate in the `Shape` is positive and a bar that clips its indicators cannot cut a corner off it. The popup was not part of this run; its own check above still stands.
