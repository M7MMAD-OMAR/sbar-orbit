...
| Working indicator | Click-through frame around the output: green edge pixels while a launch was in flight with the applications visible beneath, none after it finished, 0 to 1 CPU ticks in the panel process across both states | `ORBIT_TEST_NATIVE=1 bun run scripts/limited.ts bun run experiments/panel-check.ts` |
