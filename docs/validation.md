...
| The suite | 468 pass, 0 fail, 27 skip across 495 tests in 100 files on 22 September 2026 inside `sbarorbit.slice`, with `ORBIT_TEST_NATIVE=1` and the managed broker running alongside; the same revision without native tests reads 454 pass, 41 skip, 0 fail. The 27 skips are macOS and Windows capabilities, and the 14 native tests are exactly what `ORBIT_TEST_NATIVE=1` turns from skip to pass. The earlier measurement on this row, 458 pass, 27 skip across 485 tests in 97 files, is dated 20 September 2026. A skip is not a pass | `bun run verify`, then `ORBIT_TEST_NATIVE=1 bun run verify` |
...
Observed again on 20 September 2026, at HEAD. Two full runs, on the 485-test revision, read **458 pass, 27 skip, 0 fail**; the workspace-sweep fix then added its regression, making 486. Two runs taken while another agent held five sessions on the same shared slice each failed exactly one browser-driven test and nothing else: once `tests/native-viewer-scroll.test.ts`, whose last assertion waits up to three seconds for a reverse wheel to bring a scroll offset back to zero and found 5.02 left, which passed alone in **4.72 seconds**; and once `tests/viewer-layout.test.ts`, where `page.goto` timed out after 15 seconds loading the viewer page, which passed alone in **16.66 seconds** with 42 assertions. Both are retained as observations, and a passing rerun explains neither. That the second run failed a different test than the first is what makes this read as contention rather than as one broken test.

The Windows runner carries a flake of the same shape. On 20 September 2026 run 35505751527, at `56fd4a9`, failed `suite (windows-latest)` on `stopping a session removes its profile and its restore store from the workspace` (`tests/adversarial/reaping-and-secrets.test.ts:164`, 4984 ms), where the test polls the workspace for about 3 s and still found `profile-DWueMm`; run 35506009515, dispatched on the SAME revision with the same pinned bun 1.4.2 on Windows, passed all nine jobs. Two facts make this a budget rather than a defect: the removal is not deterministically broken at a fixed revision, and the runner is the slowest of the three. The budget was not widened for that reason. A profile that is not removed is what the test exists to catch, and loosening its timer because a slow host beat it, without measuring how long the removal actually takes there, is retrying a failure into silence.

A second Windows failure in the same window WAS a defect and is fixed: `windowsDescendants` walked one `Win32_Process` snapshot recursively with no visited set, and a snapshot can contain a cycle, which threw `RangeError: Maximum call stack size exceeded` on run 35505288681. Three tests now reproduce that on Linux in 10.20 ms, 14.87 ms and 5.03 ms against the old walk. See [release readiness](release-readiness.md).
