...
Observed again on 20 September 2026, at HEAD: two full runs read **458 pass, 27 skip, 0 fail**, and a third, taken while another agent held five sessions on the same shared slice, read **457 pass, 27 skip and one failure**. The failure was `tests/native-viewer-scroll.test.ts`, whose last assertion waits up to three seconds for a reverse wheel to bring a scroll offset back to zero and found 5.02 left; that file passed alone in **4.72 seconds** immediately afterwards. The failure is retained as an observation and a passing rerun does not explain it.
