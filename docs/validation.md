...
| The settings window and its search | Captured inside a private display: the window opens with the search focused, "boot" leaves only the Startup group standing although that word is nowhere in the window, "توهج" leaves only the three glow settings, and `sbar-orbit config set style dot` changed the mark and rebuilt the open window before the next frame | `bun run scripts/limited.ts bun run experiments/settings-window.ts` |
| Reaching a panel that is already running | `sbar-orbit settings` bound no socket, found the panel that had, handed over its request and exited zero; the window then appeared in that session. This is the path the applications menu entry uses | Same experiment |
