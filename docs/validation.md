...
| The suite | 458 pass, 0 fail, 27 skip across 485 tests in 97 files on 20 September 2026 inside `sbarorbit.slice`, with `ORBIT_TEST_NATIVE=1` and the managed broker plus other agents' sessions running alongside; the same revision without native tests reads 444 pass, 41 skip, 0 fail. The 27 skips are macOS and Windows capabilities and the native tests, which need `ORBIT_TEST_NATIVE=1` and the Fedora bootstrap. A skip is not a pass | `bun run verify`, then `ORBIT_TEST_NATIVE=1 bun run verify` |
