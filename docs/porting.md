...
| G13 | How does a queued mint reach a stopped MV3 service worker without a toast or an unbidden popup | A host with a screen, and the person at it, because the question is what appears there. The headless run that closed G12 and G14 has no screen and so cannot answer it; the one wake it used, `ServiceWorker.startWorker` from a debugger, is not a path a person has | Attempt every wake path and record which put anything on the screen. If none is clean, the honest answer is person initiated mint and the design says so |
