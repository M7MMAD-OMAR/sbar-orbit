...
| G29 | Do unprivileged user namespaces work for this on the other Linux families, and is `socat` present | **Measured 14 September 2026 in containers**, `experiments/linux-families/run.sh`, all four families: `bubblewrap` and `socat` are packaged everywhere, `max_user_namespaces` is unlimited, Orbit's own probe `bwrap --unshare-net --dev-bind / / /bin/true` passes, and inside `--unshare-net` a loopback fetch through the socat relay over a unix socket answered from outside while a direct fetch to the host's loopback was refused and a connection to 1.1.1.1 got `Network is unreachable`, with one interface in the namespace. What a rootless container cannot show: the exact Orbit shape adds `--unshare-pid --proc /proc`, and podman refuses that proc mount inside its own namespace, so the network half is measured and the pid half is the container's limit rather than the family's. Whether a distribution's AppArmor `userns` policy on a real host refuses what the container allowed is the remaining question, and it is a question about a real Ubuntu or Debian machine | Run the `confinedEgress` probe on a real host of each family, then `experiments/confined-egress.ts`. A host that refuses the sandbox must report `in-browser` and start sessions normally, which is the half that matters more than the half that works |
