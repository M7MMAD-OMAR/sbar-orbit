...
| The source tree itself | Limited | On the guest: `bun install --frozen-lockfile` exit 0 with 100 packages, `bun run typecheck` exit 0, and `bun test` **151 pass, 88 fail, 18 skip** re-measured at HEAD (was 123 / 107 / 18 before the budget landed). `RESOURCE_LIMIT_REQUIRED` now appears zero times, so the cgroup cause is closed. The remaining 88 sort into three causes by log evidence: symlinks, `/usr/bin/python3` helpers, and systemd or findmnt probes. `bin/sbar-orbit.cmd` is the Windows launcher, so the broker can be started there rather than only imported |
| Atomic version switching (`src/update.ts`) | **Refused, as currently designed** | Measured unelevated on the guest with Developer Mode absent: `symlink(dir)` is EPERM, and `rename` over a live junction is EPERM. The cause is not reparse points: `rename` over an empty PLAIN directory is EPERM too, while the same call succeeds on Linux. Windows does not replace a directory by rename, so the symlink swap does not port. A pointer FILE swapped by rename is atomic there and survived a reader holding it open; adopting it is a separate change |
| The broker | Limited | `startBroker()` bound an AF_UNIX socket under `%LOCALAPPDATA%`, answered `doctor` over it with its capability list, and stopped cleanly. `bin/sbar-orbit` is a bash script, so it was reached by importing `startBroker` directly; a Windows launcher is still to write |
