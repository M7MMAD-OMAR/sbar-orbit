...
| Reaping an owned browser when the supervisor dies | Limited | Measured through a real SIGKILL: 10 Chrome processes with a live session, supervisor killed with no cleanup handler, **0 survivors after 105 ms**. The supervisor was alive to run the sweep, so the broker's own start-up sweep is NOT exercised by this, and two best effort layers are not a kernel guarantee |
