...
| Not raising a Keychain dialog | **Reasoned, not measured** | `keychain_password_mac.mm` looks the key up by compile time constants, so a fresh `--user-data-dir` reuses the person's `Chrome Safe Storage` item and a binary off its ACL raises a modal dialog. Orbit passes `--use-mock-keychain`, documented in `os_crypt_switches.h` as existing to prevent blocking dialogs, plus `--password-store=basic`. Correct by vendor source; that no dialog appears on a person's real account is unobserved |
