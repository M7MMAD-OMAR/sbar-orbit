...
| The one command install and the broker | Limited | `./install.sh --json` exit 0, the LaunchAgent bootstrapped into `gui/$UID` and answered `launchctl print`, `doctor` exit 0 on the socket under `~/Library/Application Support`, and a whole session ran through the installed command |
| Reaping an owned browser when the supervisor dies | Limited | Measured through a real SIGKILL, four runs: 9 to 10 Chrome processes with a live session, supervisor killed with no cleanup handler, **0 survivors every time**, cleared in 86 to 480 ms. The supervisor was alive to run the sweep, so the broker's own start-up sweep is NOT exercised by this, and two best effort layers are not a kernel guarantee |
