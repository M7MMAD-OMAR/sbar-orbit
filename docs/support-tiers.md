...
| The source tree itself | Limited | On the guest: `bun install --frozen-lockfile` exit 0 with 100 packages, `bun run typecheck` exit 0, and `bun test` **158 pass, 27 fail, 87 skip** re-measured at HEAD (123 / 107 / 18 before the port's work, 151 / 88 / 18 after the budget). Most of the skip growth is suites that name themselves Linux only rather than being counted as defects. `RESOURCE_LIMIT_REQUIRED` appears zero times. The remaining 27 are classified by log pattern (14 assertions, 4 ENOENT, 3 symlink EPERM in fixtures, 2 python3 helpers, 1 systemd probe, 1 bare interpreter) and are **not** claimed to be harness only: three product bugs came out of the last two passes over that same list |
