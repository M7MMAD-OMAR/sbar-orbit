...
| The source tree itself | Limited | On the guest: `bun install --frozen-lockfile` exit 0 with 100 packages, `bun run typecheck` exit 0, and `bun test` 123 pass, 107 fail, 18 skip. The remaining failures are four named causes, not an unknown: the cgroup budget, `/usr/bin/python3` helpers, symlinks needing elevation, and systemd probes. No broker has started, because `requireResourceBudget()` refuses first |
