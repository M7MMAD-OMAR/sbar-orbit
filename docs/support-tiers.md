...
| The source tree itself | Limited | On the guest: `bun install --frozen-lockfile` exit 0 with 100 packages, `bun run typecheck` exit 0, and `bun test` **196 pass, 0 fail, 95 skip**: every test that can run on Windows does and passes, though a skip is not a pass and 95 of them are Linux capabilities, with git installed on the guest so the packaging suites really run re-measured at HEAD (123 / 107 / 18 before the port's work). Most of the skip growth is suites that name themselves Linux only rather than being counted as defects, each with a Windows case written alongside where one was owed. The 21 remaining were read one at a time: all are fixtures written against POSIX (symlinked source trees, `/bin/sh` advisors, `python3` and `findmnt` spawns, file modes, `chmod` semantics), not Windows code paths failing. That is a statement about these 21, not a general claim: four product bugs came out of three passes over this same list |
