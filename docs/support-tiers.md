...
The only class with rows that are not reasoning. Measured from `0.1.0-alpha.1` onward; each row carries its own date. The suite at HEAD on 19 September 2026: **334 pass, 0 fail, 32 skip, 366 tests across 75 files, 105 s**, inside `sbarorbit.slice`.
...
| The one command install and the broker | Limited | `./install.sh --json` exit 0, the LaunchAgent bootstrapped into `gui/$UID` and answered `launchctl print`, `doctor` exit 0 on the socket under `~/Library/Application Support`, and a whole session ran through the installed command. The suite runs **237 pass, 0 fail, 86 skip** at `3135b16`, having been 12 fail at the first run: five real defects between them, listed in [macos-measured.md](macos-measured.md) section 4 |
...
| The background scheduling class | Limited | `taskpolicy -b` is the only half of the budget the system enforces, and it is INHERITED: Orbit applied it twice until 18 September 2026, which cost a process per session and bought nothing. Measured cost of the class itself on a three core runner, alternating arms: median session 1215 ms inside it against 2032 ms outside, ratio 0.6. Not a claim that the efficiency cluster is faster; the sample is three rounds and the cause of a ratio below 1.0 is not established |
| Shared resource budget | Limited, and **advisory**, weaker than both other platforms | A registry of process groups: membership from `proc_listpids(PROC_PGRP_ONLY)` and footprint from `proc_pid_rusage`, both kernel readings, with `taskpolicy -b` as the only part the system enforces. `requireResourceBudget()` returns `enforcement: "advisory"` with every dimension in `unbounded`, and a bare `bun test` exited 1 with `RESOURCE_LIMIT_REQUIRED` on the runner. Nothing stops a process that is over the ceiling; the ceiling refuses the NEXT session. Task coalitions are the one right shaped object and are closed: creation is gated on `task_is_in_privileged_coalition()`, spawning into one needs a private Apple entitlement, and `coalition_ledger()` is root only and caps disk writes |
