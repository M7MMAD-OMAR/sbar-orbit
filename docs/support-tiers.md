...
| Owned headless browser, end to end | Limited | `launchChrome()` on the guest found Edge, published `DevToolsActivePort`, answered `/json/version` 200, opened the raw WebSocket, completed Playwright's `connectOverCDP`, navigated and read a title, then closed. A session created through the `session.create` RPC does **not** yet complete, and the launcher underneath it is not the cause |
