...
What that costs is worth being exact about, because it is paid by whoever debugs Orbit rather than by whoever uses it. A `BACKEND_ERROR` reaches its caller as the fixed words `Request failed`, and the journal records the code without the message, so the backend's own sentence exists nowhere afterwards: not in the reply, not on disk, and not in the broker's stderr, which stopped carrying it when this journal arrived. On 13 September 2026 that turned a self-closing browser tab into a day of reading a test as flaky; the underlying `click: Target page, context or browser has been closed` was only ever seen by calling the backend in process, outside the wrapper. Reproducing a failure that way, against `BrowserBackend` directly rather than through the socket, is the intended route to a cause, and the trace ID is what ties the in-process reproduction back to the recorded event. The alternative, letting the message through, is not available: a Playwright error quotes the selector it was given and a navigation error quotes the URL, so the exception text is page content by another name.
