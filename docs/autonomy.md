...
`readOnlyProfilePaths` defaults to `["Preferences", "Local State", "Secure Preferences", "Extensions"]`:
...
| Request interception over every resource type, with documents fetched one redirect hop at a time | Subresources, XHR, beacons, popups, iframes, meta refresh, javascript navigation and server side redirects | A request that never crosses the interception layer | **Limited**: built and measured after this survey. Nine page initiated routes off the allowed origin, all nine blocked; before the redirect hop check, one of nine walked through. See `experiments/origin-lease.ts`. Orbit already speaks CDP, so the cost is the round trip per request and owning `continueRequest`, `failRequest` and `continueWithAuth` |
