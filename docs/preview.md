...
Whichever browser opens it, the viewer runs in a profile that is Orbit's own and never the person's: `~/.local/state/sbar-orbit/viewer/<browser id>`, made private before the browser is asked to use it. Before 14 September 2026 the app window was opened inside the person's browser, with their extensions, cookies, history and sign-ins around it, and a window in a browser the person is also using was a tab strip away from becoming a tab. Now a Chromium build gets `--user-data-dir` on that directory, `--class=sbar-orbit-viewer`, and the two first-run flags so a fresh profile greets nobody. The class flag is kept for a plain window and does not survive app mode: measured on Hyprland with Chromium 151, the app window's class is `chrome-127.0.0.1__-Default`, which Chromium derives from the app's origin and which is still distinct from the person's browser windows, so a compositor rule matches that; a second `preview open` is forwarded to the instance on that directory, so there is one viewer browser rather than one per click, and it is not the person's instance. A Firefox fork gets `--profile` on the same directory and `--new-window`, reasoned from Firefox keying its remoting by profile since version 67 and not measured, since no Firefox build is installed here. Measured 14 September 2026 in an Orbit private display with Chromium 151: the command as the broker spawns it opens one window with app id `sbar-orbit-viewer`, no tab strip and no address bar, on that profile, and the viewer inside it reports its own per-frame cost; see [validation](validation.md).
...
There is one list of sessions, in the rail, ordered by what happened most recently, each card carrying the time of the last activity. The conversation strip that repeated that list above the picture is gone. Everything secondary is in one **Session tools** sheet, and the typing controls appear only while a person holds the controls, which is the only time they work. Orbit's own settings are a view of this page, behind one button at the foot of the rail, and the mark's right click opens it; `sbar-orbit config` still changes every one of them from a terminal. A finished session can be removed from the list with `session.forget`, which asks once in the card and leaves the session's journal on disk.
