  ...
  const server = Bun.serve({
    ...
    async fetch(request) {
      ...
      try {
        ...
        if (!["session.list", "session.observe", "session.pause", "session.resume", "session.stop", "session.control", "session.account.save"].includes(String(body.method))) throw new OrbitError("UNSUPPORTED", "Method unavailable in viewer");
